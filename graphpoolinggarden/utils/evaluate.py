...
class metricsEvaluator:
    def __init__(self, name):
        ...
        with open(os.path.join(os.path.dirname(__file__), 'dataset_info.json'), encoding='utf-8') as f:
            ...
            if self.name not in dataset_info.keys():
                error_mssg = 'Invalid dataset name {}.\n'.format(self.name)
                error_mssg += 'Available datasets are as follows:\n'
                error_mssg += '\n'.join(dataset_info.keys())
                raise ValueError(error_mssg)
            self.eval_metric = dataset_info[self.name]["metric"]
            self.num_tasks = int(dataset_info[self.name]['num_tasks'])
    ...
    def _parse_and_check_input(self, input_dict):
        if self.eval_metric == 'rocauc' or self.eval_metric == 'ap' or self.eval_metric == 'rmse' or self.eval_metric == 'acc':
            ...
            if self.eval_metric!='acc' and not y_true.shape[1] == self.num_tasks:
